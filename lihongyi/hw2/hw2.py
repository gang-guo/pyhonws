...
#
# Binary classification is one of the most fundamental problem in machine learning. In this tutorial, you are going to build linear binary classifiers to predict whether the income of an indivisual exceeds 50,000 or not. We presented a discriminative and a generative approaches, the logistic regression(LR) and the linear discriminant anaysis(LDA). You are encouraged to compare the differences between the two, or explore more methodologies. Although you can finish this tutorial by simpliy copying and pasting the codes, we strongly recommend you to understand the mathematical formulation first to get more insight into the two algorithms. Please find here.pdf) and here.pdf) for more detailed information about the two algorithms.
#
# 二元分類是機器學習中最基礎的問題之一，在這份教學中，你將學會如何實作一個線性二元分類器，來根據人們的個人資料，判斷其年收入是否高於 50,000 美元。我們將以兩種方法: logistic regression 與 generative model，來達成以上目的，你可以嘗試了解、分析兩者的設計理念及差別。針對這兩個演算法的理論基礎，可以參考李宏毅老師的教學投影片 logistic regression.pdf) 與 generative model.pdf)。
#
# 若有任何問題，歡迎來信至助教信箱 ntu-ml-2020spring-ta@googlegroups.com
#
# Dataset
...
np.random.seed(0)
X_train_fpath = 'E:/资料/学习/deeplearning/lihongyi/数据/hw2/data/X_train'
Y_train_fpath = 'E:/资料/学习/deeplearning/lihongyi/数据/hw2/data/Y_train'
X_test_fpath = 'E:/资料/学习/deeplearning/lihongyi/数据/hw2/data/X_test'
output_fpath = 'E:/资料/学习/deeplearning/lihongyi/数据/hw2/output_{}.csv'
...
def _shuffle(X, Y):
    ...
    #np.random.shuffle用于打乱ndarray的元素，只打乱第0维
...
# Please refers to Prof. Lee's lecture slides.pdf)(p.12) for the formula of gradient and loss computation.
#
# 請參考李宏毅老師上課投影片.pdf)第 12 頁的梯度及損失函數計算公式
...
# Directly compute weights and bias from in-class mean and shared variance. Prof. Lee's lecture slides.pdf)(p.33) gives a concise explanation.
#
# 權重矩陣與偏差向量可以直接被計算出來，算法可以參考李宏毅老師教學投影片.pdf)第 33 頁。
